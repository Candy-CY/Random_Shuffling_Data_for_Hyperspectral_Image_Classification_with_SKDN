...
# @Time        : 2023/10/16 16:12
# @Filename    : SKDN_TD_PU_MRViT.py
# @Author      : Candy
...
def create_data_loader(Random):
    ...
    if Random == 1:
        ...
        num_shuffle = int(finalData.shape[0] * 0.2) #0.2是随机打乱的数
        print("num_shuffle:\n",num_shuffle)
...
class ViT(nn.Module):
    def __init__(self, *, in_channels=1, image_size, patch_size, num_classes, dim, depth, heads,
        ...
        self.conv2d_features = nn.Sequential(
            nn.Conv2d(in_channels=64 * 90, out_channels=64, kernel_size=(3, 3), padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        )
        self.conv2d_features_1 = nn.Sequential(
            nn.Conv2d(in_channels=64 * 90 + 64, out_channels=110, kernel_size=(3, 3), padding=1),
            nn.BatchNorm2d(110),
            nn.ReLU(),
        )
...
if __name__ == '__main__':
    ...
    file_name = "./"+str(time)+"_0.2_MRVIT_classification_0.1_PU_epochs_200_report.txt"

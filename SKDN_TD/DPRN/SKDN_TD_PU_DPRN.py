...
def loadData(name,Random,num_components=None):
    ...
    if name == 'IP':
        ...
        if Random == 1:
            data1 = sio.loadmat(os.path.join(data_path, 'paviaU.mat'))['paviaU']
            labels1 = sio.loadmat(os.path.join(data_path, 'paviaU_gt.mat'))['paviaU_gt']
...
def main():
    ...
    parser.add_argument('--dataset', default='PU', type=str, help='dataset (options: IP, UP, SV, KSC)')
    ...
    file_name = "./"+str(time1)+"_0.1_DPRN_classification_0.1_PU_epochs_200_report.txt"

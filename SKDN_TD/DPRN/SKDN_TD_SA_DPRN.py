...
# @Time        : 2023/10/16 16:09
# @Filename    : SKDN_TD_SA_DPRN.py
# @Author      : Candy
...
def loadData(name,Random,num_components=None):
    data_path = os.path.join(os.getcwd(),'/kaggle/input/hybridsn/data')
    if name == 'IP':
        ...
        if Random == 1:
            data1 = sio.loadmat(os.path.join(data_path, 'salinas_corrected.mat'))['salinas_corrected']
            labels1 = sio.loadmat(os.path.join(data_path, 'salinas_gt.mat'))['salinas_gt']
...
def main():
    ...
    parser.add_argument('--dataset', default='SV', type=str, help='dataset (options: IP, UP, SV, KSC)')
    parser.add_argument('--tr_percent', default=0.005, type=float, help='samples of train set')
    ...
    for epoch in range(args.epochs):
        ...
        if args.use_val: test_loss, test_acc = test(val_loader_R, model, criterion, epoch, use_cuda)
        else: test_loss, test_acc = test(test_loader_R, model, criterion, epoch, use_cuda)
    ...
    test_loss, test_acc = test(test_loader_R, model, criterion, epoch, use_cuda)
    print("FINAL:      LOSS", test_loss, "ACCURACY", test_acc)
    classification,confusion,oa,aa,kappa,each_acc,results = reports(np.argmax(predict(test_loader_R, model, criterion, use_cuda), axis=1), np.array(test_loader_R.dataset.__labels__()), args.dataset)
    time1 = time.strftime("%Y-%m-%d-%H:%M:%S",time.localtime())
    file_name = "./"+str(time1)+"_0.1_DPRN_classification_0.005_SA_epochs_200_report.txt"

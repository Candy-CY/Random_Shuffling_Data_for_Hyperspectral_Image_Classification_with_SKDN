...
# @Time        : 2023/10/16 16:26
# @Filename    : SKDN_OD_SA_SSTFF.py
# @Author      : Candy
# @Email       : 2021010130@jxstnu.edu.cn
# @School      : Jiangxi Science and Technology Normal University
# @Reference   : https://github.com/zgr6010/HSI_SSFTT
# @IDE         : PyCharm
...
def loadData():
    # 读入数据
    data = sio.loadmat('/kaggle/input/hybridsn/data/salinas_corrected.mat')['salinas_corrected']
    labels = sio.loadmat('/kaggle/input/hybridsn/data/salinas_gt.mat')['salinas_gt']
    return data, labels
...
def create_data_loader(Random):
    ...
    if Random == 1:
        ...
        print('\n... ... PCA tranformation ... ...')
        X_pca = applyPCA(X, numComponents=pca_components)#(NewData,numComponents=pca_components)#
        print('Data shape after PCA: ', X_pca.shape)
        print('\n... ... create data cubes ... ...')
        X_pca, y_all = createImageCubes(X_pca, y, windowSize=patch_size)#(X_pca,Newlabel,windowSize=patch_size)#
        print('Data cube X shape: ', X_pca.shape)
        print('Data cube y shape: ', y_all.shape)
        print('\n... ... create train & test data ... ...')
        Xtrain, Xtest, ytrain, ytest = splitTrainTestSet(X_pca, y_all, test_ratio)
...
if __name__ == '__main__':
    ...
    net, device = train(train_loader,train_loader_Random, epochs=100)
    # 只保存模型参数
    ...
    file_name = "./"+str(time)+"_0.1_SSFTT_classification_0.005_SA_epochs_100_report.txt"
